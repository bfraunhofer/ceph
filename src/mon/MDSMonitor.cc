			    ...
			    list<pair<health_status_t, string> > *detail) const
{
  ...
  for (std::map<uint64_t, MDSMap::mds_info_t>::const_iterator i = pending_mdsmap.mds_info.begin();
    ...
    if (health.log_segment_count > health.log_max_segments * 2) {
      // Use rank rather than GID to refer to MDS for the user
      std::ostringstream oss;
      oss << "MDS " << i->second.rank << " is behind on trimming (" << health.log_segment_count
        << "/" << health.log_max_segments << ")";
      summary.push_back(std::make_pair(HEALTH_WARN, oss.str()));
    }

      std::ostringstream oss;
      oss << "TEST " << i->first << " - " << i->second.rank << " is behind on trimming (" << health.log_segment_count
        << "/" << health.log_max_segments << ")";
      summary.push_back(std::make_pair(HEALTH_WARN, oss.str()));
  }
}
